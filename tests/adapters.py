...
from cs336_basics.modules import Linear, Embedding, MultiHeadSelfAttention, RMSNorm, silu, SwiGLU, RoPE, scaled_dot_product_attention, softmax, TransformerBlock, TransformerLM
...
def run_multihead_self_attention_with_rope(
    ...
    RoPE_instance = RoPE(theta=theta, d_k=d_model // num_heads, max_seq_len=max_seq_len)
    ...
    return mha(in_features, rope=RoPE_instance, token_positions=token_positions)
...
def run_rope(
    ...
    output = RoPE_layer(in_query_or_key, token_positions)
...
def run_transformer_block(
    ...
    return block(in_features, rope=RoPE_instance, token_positions=token_positions)
...
def run_cross_entropy(
    ...
    safe_logits = inputs - max_logits
    
    exp_logits = torch.exp(safe_logits)
    sum_exp_logits = torch.sum(exp_logits, dim=1, keepdim=True)
    log_probs = safe_logits - torch.log(sum_exp_logits)
